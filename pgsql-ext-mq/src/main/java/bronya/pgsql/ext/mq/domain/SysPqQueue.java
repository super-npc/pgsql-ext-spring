...
import bronya.pgsql.ext.mq.proxy.SysPqQueueProxy;
...
@Ignore
@FieldNameConstants
@Amis(dynamicTablePlan = PgMqADynamicTablePlan.class,ext = @Amis.ExpandField(extBean = SysPqQueue.SysPqQueueExt.class,dataProxy = SysPqQueueProxy.class))
@AmisPage(menu = @Menu(show = false, module = 系统.class, group = postgres.class, menu = "pg队列",order = 1),
        orderBys = {@AmisPage.OrderBy(cols = {SysPqQueue.Fields.msgId}, type = OrderByType.DESC)},
        btns = @Btns(add = false, edit = false, delete = false))
public class SysPqQueue{
    ...
    @BindMany2One(entity = SysPqMeta.class, valueField = Fields.msgId, labelField = SysPqMeta.Fields.queueName)
    @AmisField
    private Long pgQueue;

    @Data
    public static class SysPqQueueExt{
